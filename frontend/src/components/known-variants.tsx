...
export default function KnownVariants({
    ...
    const analyzeVariant = async (variant: ClinvarVariant) => {
        ...
        const position = variant.location ? parseInt(variant.location.replace(",", "")) : null;
                                ...
                                {clinvarVariants.map((variant) => (
                                    ...
                                    >
                                        <TableCell className="py-2">
                                            <div className="text-sm font-medium text-[#3c4f3d]/70">
                                                {variant.title}
                                            </div>
                                            <div className="mt-1 flex items-center gap-1 text-sm text-[#3c4f3d]/70">
                                                <p>Location {variant.location}</p>
                                                <Button
                                                    variant="link"
                                                    size="sm"
                                                    className="h-6 cursor-pointer px-0 text-[#de8246] hover:text-[#de8246]/80"
                                                    onClick={() => window.open(
                                                        `https://www.ncbi.nlm.nih.gov/clinvar/variation/${variant.clinvar_id}`, "_blank")}
                                                >
                                                    View in ClinVar
                                                ...
                                                {variant.variation_type.toLowerCase().includes("single nucleotide") ? (
                                                    !variant.evo2Result ? (
                                                        ...
                                                        >
                                                            {variant.isAnalyzing ? (
                                                                <>
                                                                   <span className="h-3 w-3 animate-spin rounded-full border-2 border-[3c4f3d]/30 border-t-[#3c4f3d]"></span>
                                                                    Analyzing...
                                                                </>
                                                            ) : (
                                                                <>
                                                                    <Zap className="mr-1 h-3 w-3 inline-block" />
                                                                    Analyse with evo2
                                                                </>
                                                            )}
                                                        </Button>
